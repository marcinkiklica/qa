...
public class WindowsTest extends ConfigFrontend {
    ...
    public void testWindows() {
        ...
        By byMainFrame = By.id("mywindowframe");

        new WebDriverWait(driver, 10)
                .until(ExpectedConditions.visibilityOfElementLocated(byMainFrame));

        firstPageWindowHandle = driver.getWindowHandle();

        int hyperlinkYCoordinate = driver.findElement(byMainFrame).getLocation().getY();
        int hyperlinkXCoordinate = driver.findElement(byMainFrame).getLocation().getX();

        JavascriptExecutor jsexecutor = (JavascriptExecutor) driver;
        jsexecutor.executeScript("window.scrollBy(" + hyperlinkXCoordinate + "," + hyperlinkYCoordinate + ")", "");

        new WebDriverWait(driver, 100)
                .until(ExpectedConditions.elementToBeClickable(byMainFrame));

        driver.findElement(byMainFrame).click();
